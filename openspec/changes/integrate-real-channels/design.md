...
- **[Risk]** SMS delivery depends on Twilio account-level registration (10DLC or toll-free verification) that is slow and entirely outside this codebase's control. → **Mitigation**: build and test the Twilio integration against trial/sandbox credentials first (phase 6); treat production A2P registration as a parallel, non-blocking track rather than a gate on shipping the code.
...
6. **Twilio (SMS)** — swap the mock SMS sender for a real Twilio call.
