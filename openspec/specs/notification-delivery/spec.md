...
Asynchronous, queue-based dispatch of notifications from acceptance through the delivery lifecycle (`queued` → `processing` → `delivered`/`failed`), including retry behavior for transient failures.
