...
| SMS delivery | Stays mocked (`src/channels/sms.ts`), unlike email/push | A real Twilio integration was built and tested (dependency, config, real API call, mocked-client tests) but reverted. Singapore's IMDA SMS Sender ID Registry requires every SMS provider — not just Twilio — to have a registered alphanumeric Sender ID before delivering to any Singapore number, or the message is blocked/flagged "Likely-SCAM". The only realistic test destination available was a Singapore number, and that registration needs local business documentation outside this project's scope. Revisit if a non-Singapore test destination becomes available, or the registration is completed. |
